...
function BeforeSignUp() {
  ...
  const navLinkDisplay = () => {
    if (accountType === "IND") {
      return (
        <NavLink className=" BeforeSignUp-button  px-4 py-2  text-white no-underline text-center" to="/signUpIndividual">
          Next
        </NavLink>
      );
    } else {
      return (
        <NavLink className=" BeforeSignUp-button  px-4 py-2  text-white no-underline text-center" to="/signUpOrganisation">
          Next
        </NavLink>
      );
    }
        ...
        <div className="bg-white flex flex-col justify-around items-center w-11/12">
          ...
          >
            <div className=" font-sans text-2xl">
              <input type="radio" id="individual" value="individual" name="signUp-Type" className="mr-3 mb-3" />
              <label htmlFor="individual">Individual</label>
            </div>
            <div className=" font-sans text-2xl">
              <input type="radio" id="organisation" value="organisation" name="signUp-Type" className="mr-3" />
              <label htmlFor="organisation">Organisation</label>
            </div>
          </div>

          <div className=" BeforeSignUp-text font-sans text-xl text-blue-600">Already having account ? Sign In</div>
          <div className="p-4">{navLinkDisplay()}</div>
        </div>
        <div className="text-center text-blue-700 absolute bottom-0 mb-3 ">
          <NavLink className="text-blue-700 font-sans text-xl mb-2 no-underline" to="/homePage">
            ← Back to Home
          </NavLink>
        </div>
      </div>
